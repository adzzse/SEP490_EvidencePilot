...
function SettingsSection({ api }) {
  ...
  const handleCatDelete = (c) => {
    ...
    startDelete({
      entityName: c.name,
      entityDetails: c.id,
      header: t('admin.undoHeader'),
      bodyTemplate: t('admin.undoBodyTemplate'),
  ...
  const getConfigSecurity = (key) => {
    ...
    if (k.includes('jwt') || k.includes('secret') || k.includes('password')) return t('admin.secretLabel');
    if (k.includes('url') || k.includes('port')) return t('admin.internalLabel');
    return t('admin.publicLabel');
  };
  ...
  return (
    <div className="p-8 space-y-6 bg-(--page-bg)">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-(--border) pb-5">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-(--brand-foreground) tracking-tight">{t('admin.settings')}</h1>
          <p className="text-gray-550 text-xs mt-1">{t('admin.settingsSub')}</p>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b border-(--border-light) pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xs font-bold text-(--text-primary) uppercase tracking-wider">{t('admin.collectionCategories')}</h3>
              </div>
              <button
            ...
            {catsLoading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-6 px-4 border border-dashed border-(--border) rounded-xl bg-(--surface-secondary)/50">
                ...
                <span className="font-bold text-xs text-(--text-primary)">{t('admin.noCategoryYet')}</span>
                <p className="text-[10px] text-(--text-tertiary) font-semibold mt-0.5 leading-relaxed max-w-xs">{t('admin.noCategoryYetHint')}</p>
              </div>
            ) : (
                ...
                {cats.map(c => (
                  <div key={c.id} className="flex items-center justify-between py-2 hover:bg-(--surface-secondary)/50 rounded px-1 transition">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => { setCatForm({ id: c.id, name: c.name, description: c.description || '' }); setShowCatForm(true); }} className="px-2 py-1 text-[10px] font-bold text-(--text-secondary) bg-(--surface-secondary) border border-(--border) rounded-lg hover:bg-(--surface-tertiary) hover:text-(--text-primary) transition cursor-pointer">{t('admin.edit')}</button>
                      <DeleteConfirm
        ...
        <div className="px-6 py-4.5 border-b border-(--border-light) flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-(--text-primary) tracking-wider uppercase">{t('admin.systemConfiguration')}</h3>
            </div>
            <p className="text-[10px] text-(--text-tertiary) font-semibold mt-1">🔓 {t('admin.systemSettingsHint')}</p>
          </div>
          <button
          ...
          >
            ...
            <span>{t('admin.exportEnvFile')}</span>
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-(--surface-secondary) text-(--text-tertiary) font-bold uppercase border-b border-(--border-light)">
                  <th className="px-6 py-3.5">{t('admin.variableName')}</th>
                  <th className="px-6 py-3.5">{t('admin.runtimeValue')}</th>
                  <th className="px-6 py-3.5 text-right">{t('admin.security')}</th>
                </tr>
              </thead>
      ...
      {showCatForm && (
        <div className="fixed inset-0 z-55 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-(--surface) rounded-2xl shadow-2xl p-6 w-full max-w-md border border-gray-150 transform scale-100 transition-all duration-300">
            <h3 className="font-bold text-(--text-primary) text-sm mb-4">{catForm.id ? t('admin.editCollectionCategory') : t('admin.addCollectionCategory')}</h3>
            <form onSubmit={doCatSave} className="space-y-4">
              <div>
                <label className="text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider block mb-1.5">{t('admin.categoryNameRequired')}</label>
                <input
              ...
              <div>
                <label className="text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider block mb-1.5">{t('admin.description')}</label>
                <textarea
              ...
              <div className="flex gap-2.5 justify-end pt-2">
                <button type="button" onClick={() => setShowCatForm(false)} className="px-3.5 py-2 text-xs font-bold text-slate-605 hover:bg-(--surface-secondary) rounded-xl transition cursor-pointer">{t('admin.cancel')}</button>
                <button type="submit" className="px-4 py-2 bg-[#0c162e] hover:bg-[#152447] text-white rounded-xl text-xs font-bold transition shadow-md cursor-pointer">{catForm.id ? t('admin.saveChanges') : t('admin.addCategory')}</button>
              </div>
            </form>
          </div>
        </div>
