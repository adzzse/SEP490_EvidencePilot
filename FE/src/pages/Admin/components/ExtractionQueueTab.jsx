  ...
  return (
    <div className="p-8 space-y-6 bg-(--page-bg)">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-(--border) pb-5">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-(--brand-foreground) tracking-tight">{lang.extractionQueue}</h1>
          <p className="text-(--text-secondary) text-xs mt-1">{lang.queueSub}</p>
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-(--surface) rounded-xl border border-(--border) p-5 shadow-sm flex flex-col justify-between h-36">
          ...
          <div className="mt-2">
            <span className="text-[10px] font-bold text-(--text-tertiary) block tracking-wider uppercase">Total in Queue</span>
        ...
        <div className="bg-(--surface) rounded-xl border border-(--border) p-5 shadow-sm flex flex-col justify-between h-36">
          ...
          <div className="mt-2">
            <span className="text-[10px] font-bold text-(--text-tertiary) block tracking-wider uppercase">Ready for Extraction</span>
        ...
        <div className="bg-(--surface) rounded-xl border border-(--border) p-5 shadow-sm flex flex-col justify-between h-36">
          ...
          <div className="mt-2">
            <span className="text-[10px] font-bold text-(--text-tertiary) block tracking-wider uppercase">Currently Processing</span>
        ...
        <div className="bg-(--surface) rounded-xl border border-(--border) p-5 shadow-sm flex flex-col justify-between h-36">
          ...
          <div className="mt-2">
            <span className="text-[10px] font-bold text-(--text-tertiary) block tracking-wider uppercase">Total Failed</span>
      ...
      <div className="bg-(--surface) rounded-2xl border border-(--border) shadow-sm overflow-hidden">
        ...
        <div className="px-6 py-4.5 border-b border-(--border-light) flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            <h3 className="text-lg font-bold text-(--text-primary)">{activeTab === 'Failed' ? lang.failedDocuments : `${activeTab} Documents`}</h3>
            {/* Status Tabs */}
        ...
        {detailDoc && (
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <span className="text-[10px] font-bold text-(--text-tertiary) uppercase tracking-wider block">{lang.title}</span>
                <span className="font-bold text-(--text-primary) break-words">{detailDoc.originalFilename}</span>
              </div>
              <div>
                <span className="text-[10px] font-bold text-(--text-tertiary) uppercase tracking-wider block">{lang.project}</span>
                <span className="font-bold text-(--text-primary)">{detailDoc.project}</span>
              </div>
              <div>
                <span className="text-[10px] font-bold text-(--text-tertiary) uppercase tracking-wider block">{lang.extractionStatus}</span>
            ...
            <div className="bg-(--surface-secondary) border border-(--border) rounded-xl p-4 min-w-0">
              <span className="text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider block mb-2">Document details</span>
