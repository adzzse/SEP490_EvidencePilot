...
import SourceLibraryContent from '../../components/Student/SourceLibraryContent.jsx';
...
function findingClassName(finding) {
  ...
  const hasSupport = evidence.some(item => item.relation === 'SUPPORTS');
  const parts = ['cm-review-finding'];
  if (finding.type === 'SOURCE_DISCREPANCY') parts.push('cm-review-finding--discrepancy');
  else if (!hasSupport) parts.push('cm-review-finding--unsupported');
  const confidence = String(finding.confidence || '').toLowerCase();
  if (['high', 'medium', 'low'].includes(confidence)) parts.push(`cm-review-finding--${confidence}`);
  return parts.join(' ');
}
...
export default function WorkspaceLayout({ workspaceMode = 'student' }) {
  ...
  const workspace = isReview ? review.workspace : {
    ...studentWorkspace,
    project: studentProject,
    papers: studentPapers,
    sources: studentSources,
    mediaAssets: studentMediaAssets,
    sections: studentSections,
    selectedPaper: studentSelectedPaper,
    selectedPaperId: studentSelectedPaper?.id || null,
    selectedSectionId: studentSelectedSectionId,
  ...
  const [fileTreeWidth, setFileTreeWidth] = useState(() => {
    const stored = Number(localStorage.getItem('student_filetree_width'));
    return Number.isFinite(stored) && stored >= 200 && stored <= 450 ? stored : 280;
  });
  ...
  const sections = workspace.sections || studentSections;
  const selectedSectionId = isReview ? workspace.selectedSectionId : studentSelectedSectionId;
  ...
  const editorRef = isReview ? workspace.editorRef : studentEditorRef;
  ...
  const putSectionContent = useCallback((paperId, sectionId, content, fallbackRevision, changes) => {
    ...
    const next = prev.catch(() => undefined).then(async () => {
      ...
      const response = await api.put(
        ...
        { content, expectedRevision, ...(changes ? { changes } : {}) },
      );
  ...
  const handleSelectSection = async (sec) => {
    if (isReview) {
      workspace.selectSection(sec.id);
  ...
  const handleSelectFeedback = async item => {
    if (String(item.sectionId) !== String(selectedSectionIdRef.current)) {
      const section = sections.find(entry => String(entry.id) === String(item.sectionId));
      if (section) {
        if (!await handleSelectSection(section)) return false;
      } else {
        const paper = papers.find(entry => String(entry.id) === String(item.paperId));
        if (!paper) { showToast(t('studentFeedback.sectionUnavailable')); return false; }
        const current = selectedSectionIdRef.current;
        if (current && dirtySectionsRef.current.has(current)) {
          if (!window.confirm(t('unsavedSectionSwitch'))) return false;
          dirtySectionsRef.current.delete(current);
          removeWorkspaceDraft(projectRef.current?.id, current);
        }
    ...
    if (String(item.sectionId) === String(selectedSectionId)) {
      ...
      if (position?.from != null) {
        editorRef.current?.revealRange(position.from, position.to);
        pendingFeedbackRef.current = null;
      }
    }
    return true;
  ...
  const handleSelectPaper = async (p) => {
    if (isReview) { workspace.selectPaper(p.id); return; }
    const current = selectedSectionIdRef.current;
    if (current && dirtySectionsRef.current.has(current)) {
  ...
  const displayContent = isReview ? workspace.content : selectedPaper ? codeContent : `% ${t('noPaper')}`;
  ...
  const handleLeftDividerMouseDown = (e) => {
    ...
    const onMouseUp = () => { document.removeEventListener('mousemove', onMouseMove); document.removeEventListener('mouseup', onMouseUp); document.body.style.cursor = ''; document.body.style.userSelect = ''; if (latest != null) { try { localStorage.setItem('student_filetree_width', String(Math.round(latest))); } catch { /* ignore */ } } };
    document.addEventListener('mousemove', onMouseMove);
    document.addEventListener('mouseup', onMouseUp);
  };
  ...
  // Inline citation review overlay state.
  // Anchor pixels arrive from the CM widget's click-time coordsAtPos — never queried here.
  const [reviewOverlay, setReviewOverlay] = useState({ open: false, findingIndex: -1, anchor: null });
  const isReviewOverlayOpen = reviewOverlay.open;
  ...
  const retrievableReferenceSourceIds = useMemo(
    () => new Set(paperReferences.filter(reference => reference.retrievable).map(reference => String(reference.sourceId))),
    [paperReferences],
  );
  const canMutateReferences = !isReview && !isLocked;

  const refreshReferences = useCallback(async () => {
    paperRefs.clearCheck();
    await paperRefs.reloadReferences();
  ...
  const citationIndex = useMemo(() => {
    ...
    Object.values(aiSourceMatches || {}).forEach(candidates => {
      (candidates || []).forEach(candidate => {
        if (candidate.citationKey && !map[candidate.citationKey]) {
          map[candidate.citationKey] = {
    ...
    return map;
  }, [paperReferences, aiSourceMatches]);
    ...
    api.get(`/api/papers/${selectedPaper.id}/sections`, { signal: controller.signal })
      .then(r => {
        ...
        if (target) {
          ...
          if (draft !== null) {
            loadCode(draft);
            dirtySectionsRef.current.add(target.id);
          } else {
            loadCode(target.contentTex || '');
          }
        } else {
          selectSection('');
          loadCode('');
  ...
  useEffect(() => {
    ...
    if (!pending || String(pending.sectionId) !== String(selectedSectionId)) return;
    let frame;
    let attempts = 0;
    const reveal = () => {
      const position = editorRef.current?.getFeedbackPositions?.().find(item => item.id === pending.id);
      if (!position && attempts++ < 20) { frame = requestAnimationFrame(reveal); return; }
      if (position?.from != null) editorRef.current?.revealRange(position.from, position.to);
      if (pendingFeedbackRef.current === pending) pendingFeedbackRef.current = null;
    };
  ...
  const handleRemoveReference = useCallback(async (sourceId) => {
    try {
      ...
      showToast(error?.response?.status === 409 ? t('referenceRemoveCited') : t('failedToRemoveReference'));
    }
  ...
  const handleDeleteSource = async (sourceId) => {
    ...
    startDelete({
      ...
      setSources(await loadAllProjectSources(project.id));
      if (deleted) await refreshReferences();
    });
  };
  ...
  const handleSaveDraft = async () => {
    ...
    const changes = snapshot?.content === normalizeSource(content) ? snapshot.changes : undefined;
    const revision = sections.find(section =>
      String(section.id) === String(sectionId))?.revision;
    try {
      const updated = await putSectionContent(
        paperId, sectionId, content, revision, changes);
  ...
  const handleRunReferenceCheck = async () => {
    ...
    try {
      const result = await paperRefs.runCheck();
      if (!result) return;
      setActiveTab('Source');
      setIsDrawerOpen(true);
    } catch {
  ...
  const handleInsertReviewCitation = async (finding, candidate) => {
    ...
    if (!isReferenceCandidate(candidate, referenceSourceIds)) {
      showToast(t('citationNotInReferences'));
      return;
    }
    if (!isReferenceCandidate(candidate, retrievableReferenceSourceIds)) {
      showToast(t('referenceNotRetrievable'));
      return;
    }
  ...
  const insertLatexTag = (tagType) => {
    ...
    else if (tagType === 'label') { const n = prompt(t('student.workspace.labelNamePrompt'), 'sec:label') || 'sec:label'; insertion = `\\label{${n}}`; offset = insertion.length; }
    else if (tagType === 'cite') { const k = prompt(t('student.workspace.citationKeyPrompt'), 'author2026') || 'key'; insertion = `\\cite{${k}}`; offset = insertion.length; }
    else if (tagType === 'link') { const url = prompt(t('student.workspace.linkUrlPrompt'), 'https://') || 'https://'; const l = sel || prompt(t('student.workspace.linkLabelPrompt'), t('student.workspace.defaultLinkLabel')) || t('student.workspace.defaultLinkLabel'); insertion = `\\href{${url}}{${l}}`; offset = insertion.length; }
  ...
  const reviewAction = isReview ? null : {
    ...
    description: isReferenceSection
      ? t('refCheckDescription')
      : isAbstractSection
    ...
    error: isReferenceSection
      ? (paperRefs.checkError ? t(paperRefs.checkError) : null)
    ...
    <div role="region" aria-label={t('feedbackProjectWorkspace')} className="h-screen w-full flex flex-col bg-(--surface-secondary) overflow-hidden font-sans antialiased text-(--text-primary)">
      ...
      <div id="workspace-container" className="relative flex-1 flex overflow-hidden min-w-0">
        <div data-tour="sidebar-left" className="w-14 bg-indigo-900 dark:bg-(--accent-bar) flex flex-col items-center py-4 shrink-0 z-20 border-r border-indigo-950 dark:border-(--border) shadow-[2px_0_8px_-2px_rgba(0,0,0,0.2)]">
          ...
          {!isReview && <button onClick={toggleContextPanel} className="w-full flex justify-center cursor-pointer mb-6 group relative" title={t('toggleContextPanel')} aria-pressed={isDrawerOpen}>
          ...
          <div className="bg-(--surface) rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {editableSections.length === 0 ? (
                ...
                const sec = editableSections.find(s => String(s.id) === String(selectedSectionId)) || editableSections[0];
                return sec ? (
                  <>
      ...
      <SubmissionReadinessModal
        ...
        submittedPending={feedback.requests.some(request => request.status === 'PENDING')}
        onSubmitted={async () => {
      ...
      <InlineCitationCard
        ...
        retrievableReferenceSourceIds={retrievableReferenceSourceIds}
      ...
      {showFullPaperPreview && (
        <FullPaperPreview
          ...
          mediaAssets={mediaAssets} paperReferences={paperReferences}
          onClose={() => setShowFullPaperPreview(false)}
          onAnnotateSection={isReview && review.workflow.canCreateRoot ? id => { workspace.selectSection(id); setShowFullPaperPreview(false); setActiveTab('Review'); setIsDrawerOpen(true); } : undefined}
        />
      )}
    </div>
  );
