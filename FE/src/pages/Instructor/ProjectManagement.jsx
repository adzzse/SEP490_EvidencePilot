...
export default function ProjectManagement() {
  ...
  const handleDelete = (id) => {
    ...
    const projTitle = proj?.title || id;
    startDelete({
      entityName: projTitle,
      entityDetails: id,
    }, async () => {
      setDeletingId(id);
      try {
              ...
              >
                    ...
                    <div className="flex items-center gap-1.5">
                      {p.deletionScheduledAt && (
                        <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-bold text-amber-800">
                          {t('instructor.projectManagement.deletionScheduled')}
                        </span>
                      )}
                ...
                <div className="border-t border-(--border-light) pt-3">
                  ...
                  <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                    ...
                    {hasProjectAction(actionState(p), 'revokeDeletion') && <button onClick={() => handleRevokeDeletion(p.id)} className="text-xs text-amber-600 hover:bg-amber-50 font-bold px-2.5 py-1 rounded-lg transition-colors cursor-pointer">{t('instructor.projectManagement.revokeDeletion')}</button>}
            ...
            {projects.map(p => (
              <div key={p.id} data-testid={`project-card-${p.id}`} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 hover:bg-(--surface-secondary) transition-colors">
                ...
                <div className="flex flex-wrap gap-1 sm:justify-end" onClick={e => e.stopPropagation()}>
                  ...
                  {hasProjectAction(actionState(p), 'revokeDeletion') && <button onClick={() => handleRevokeDeletion(p.id)} className="text-xs text-amber-600 hover:text-amber-800 font-bold px-2 py-1.5 cursor-pointer">{t('instructor.projectManagement.revokeDeletion')}</button>}
