...
export default function InstructorDashboard() {
  ...
  const startTour = async () => {
    ...
    void _css;
    const tourCopy = t('instructor.dashboard.tourSteps', { returnObjects: true });
    const steps = [
      { element: '#metrics-grid', popover: { ...tourCopy[0], side: 'bottom', align: 'start' } },
      { element: '#operational-hub', popover: { ...tourCopy[1], side: 'top', align: 'start' } },
      { element: '#dashboard-tour-btn', popover: { ...tourCopy[2], side: 'bottom', align: 'end' } },
      { element: '#review-queue-link', popover: { ...tourCopy[3], side: 'left', align: 'start' } },
    ];
    const d = driver({
      steps,
      showProgress: true,
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-8 border-b border-(--border) pb-5">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-(--brand-foreground) tracking-tight">
              {t('instructor.dashboard.instructorControlDashboard')}
            </h1>
            <p className="text-(--text-secondary) text-sm mt-1">
              {t('instructor.dashboard.instructorDashboardDesc')}
            </p>
          </div>
          ...
          >
            ...
            {t('instructor.dashboard.userGuidance')}
          </button>
        </div>
        ...
        <div id="metrics-grid" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-4 gap-5 mb-8">
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-(--text-tertiary)">{t('instructor.dashboard.projects')}</span>
            ...
            <div className="mt-4">
              ...
              <p className="text-[11px] text-(--text-tertiary) mt-1 font-medium">{t('instructor.dashboard.manageProjects')} &rarr;</p>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-(--text-tertiary)">{t('instructor.dashboard.collections')}</span>
            ...
            <div className="mt-4">
              ...
              <p className="text-[11px] text-(--text-tertiary) mt-1 font-medium">{t('instructor.dashboard.manageCollectionsLink')} &rarr;</p>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-(--text-tertiary)">{t('instructor.dashboard.reviewRequests')}</span>
            ...
            <div className="mt-4">
              <div className="text-2xl lg:text-3xl font-black text-(--text-primary) flex items-center gap-2">
                ...
                {!loadingMetrics && metrics.pendingRequestsCount > 0 && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-500/20 text-amber-600">{t('instructor.dashboard.actionRequired')}</span>
                )}
              </div>
              <p className="text-[11px] text-(--text-tertiary) mt-1 font-medium">{t('instructor.dashboard.reviewSubmissions')} &rarr;</p>
            </div>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-(--text-tertiary)">{t('instructor.dashboard.sourceLibrary')}</span>
            ...
            <div className="mt-4">
              ...
              <p className="text-[11px] text-(--text-tertiary) mt-1 font-medium">{t('instructor.dashboard.viewSourceLibrary')} &rarr;</p>
            </div>
          </div>
        ...
        <div id="operational-hub" className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-black text-(--brand-foreground)">
              {t('instructor.dashboard.operationalHub')}
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-4 gap-5">
            ...
            <div className="bg-(--surface) border border-(--border) rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:border-(--brand)/40 transition-colors">
              <div>
                ...
                <h3 className="text-sm font-black text-(--brand-foreground) mb-1">{t('instructor.dashboard.projectManager')}</h3>
                <p className="text-xs text-(--text-secondary) leading-relaxed mb-3 line-clamp-2">{t('instructor.dashboard.projectManagerDesc')}</p>
              </div>
              <Link to="/instructor/projects" className="inline-flex items-center text-xs font-bold text-(--brand) hover:underline gap-1 pt-2 border-t border-(--border-light)">
                {t('instructor.dashboard.manageProjects')} &rarr;
              </Link>
            </div>
            ...
            <div className="bg-(--surface) border border-(--border) rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:border-(--brand)/40 transition-colors">
              <div>
                ...
                <h3 className="text-sm font-black text-(--brand-foreground) mb-1">{t('instructor.dashboard.reviewRequests')}</h3>
                <p className="text-xs text-(--text-secondary) leading-relaxed mb-3 line-clamp-2">{t('instructor.dashboard.reviewRequestsDesc')}</p>
              </div>
              <Link id="review-queue-link" to="/instructor/requests" className="inline-flex items-center text-xs font-bold text-(--brand) hover:underline gap-1 pt-2 border-t border-(--border-light)">
                {t('instructor.dashboard.reviewSubmissions')} &rarr;
              </Link>
            </div>
            ...
            <div className="bg-(--surface) border border-(--border) rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:border-(--brand)/40 transition-colors">
              <div>
                ...
                <h3 className="text-sm font-black text-(--brand-foreground) mb-1">{t('instructor.dashboard.collectionsManager')}</h3>
                <p className="text-xs text-(--text-secondary) leading-relaxed mb-3 line-clamp-2">{t('instructor.dashboard.collectionsManagerDesc')}</p>
              </div>
              <Link to="/instructor/collections" className="inline-flex items-center text-xs font-bold text-(--brand) hover:underline gap-1 pt-2 border-t border-(--border-light)">
                {t('instructor.dashboard.manageCollectionsLink')} &rarr;
              </Link>
            </div>
            ...
            <div className="bg-(--surface) border border-(--border) rounded-2xl p-5 shadow-xs flex flex-col justify-between hover:border-(--brand)/40 transition-colors">
              <div>
                ...
                <h3 className="text-sm font-black text-(--brand-foreground) mb-1">{t('instructor.dashboard.sourceLibrary')}</h3>
                <p className="text-xs text-(--text-secondary) leading-relaxed mb-3 line-clamp-2">{t('instructor.dashboard.sourceLibraryDesc')}</p>
              </div>
              <Link to="/instructor/source-library" className="inline-flex items-center text-xs font-bold text-(--brand) hover:underline gap-1 pt-2 border-t border-(--border-light)">
                {t('instructor.dashboard.viewSourceLibrary')} &rarr;
              </Link>
            </div>
