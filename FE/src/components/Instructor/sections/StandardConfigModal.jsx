...
export function StandardConfigEditor({ open = true, initialRequirements = [], isLocked, onSave, onClose, t, ct }) {
    ...
    <div className="space-y-4 text-xs">
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2">
          <input data-testid="standard-requirement-input" value={input} maxLength={250} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{ if(e.key==='Enter'){ e.preventDefault(); addReq(); }}} disabled={isLocked || saving} placeholder={t.addStandardRequirement} aria-label={t.addStandardRequirement} className="flex-1 rounded border border-[var(--border)] px-2 py-1 outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus)] disabled:opacity-50" />
