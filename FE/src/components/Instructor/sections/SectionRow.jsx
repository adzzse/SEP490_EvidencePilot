...
export default function SectionRow({
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      className={`flex items-center justify-between gap-3 rounded-lg px-3 py-3 text-xs sm:px-4 ${isConflict ? 'ring-2 ring-amber-400 bg-amber-50 border border-amber-300' : isDragging ? 'border border-indigo-200 bg-[var(--brand-soft)] shadow-lg opacity-60' : 'bg-[var(--surface-secondary)]'
        }`}
    >
      <div className="flex min-w-0 flex-1 items-center gap-3">
        <span
          className={`shrink-0 text-[var(--text-tertiary)] ${isLocked ? 'cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}`}
        ...
        {isEditing ? (
          <div className="flex min-w-0 flex-1 items-center gap-1">
            <input
              ...
              className="min-w-0 flex-1 bg-transparent outline-none border-b border-indigo-300 text-xs"
            />
          ...
          <span className="min-w-0 flex-1 truncate font-medium" title={s.sectionTitle}>{s.sectionTitle}</span>
        )}
        {s.version > 1 && <span className="shrink-0 text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded font-bold">v{s.version}</span>}
        {s.assignedUserId && (
          <span className="flex shrink-0 items-center gap-1 rounded bg-[var(--surface-tertiary)] px-1.5 py-0.5 text-[9px] font-bold text-[var(--text-secondary)]">
        ...
        {isLocked && (
          <button
            ...
            className="shrink-0 rounded p-1 text-indigo-600 hover:bg-indigo-50 disabled:opacity-50"
          >
