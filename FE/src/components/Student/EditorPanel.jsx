...
export default function EditorPanel({
  ...
  const canRunCitationReview = !review && !isLocked && Boolean(selectedPaper && selectedSectionId && isOwnSection);
  const citationReviewTitle = reviewBusy ? t('reviewing') : canRunCitationReview ? t('citationReviewDescription') : t('citationReviewUnavailable');
  ...
  useEffect(() => { setShowPreview(false); }, [selectedSectionId, review?.activeFeedbackId]);
      ...
      <div style={{ flex: narrow ? '1 1 0' : threePanes ? '1 1 480px' : `${editorWidth} 1 0` }} className={`bg-(--surface) rounded-lg shadow-sm border border-(--border) ${narrow && previewVisible ? 'hidden' : 'flex'} flex-col overflow-hidden min-w-0 min-h-0`}>
        <div data-tour="editor-toolbar" className="h-10 border-b border-(--border-light) flex items-center justify-between px-3 bg-(--surface) shadow-sm shrink-0 z-10">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            {!review && <span data-tour="editor-ai-review" className="inline-flex" title={citationReviewTitle}>
              <button type="button" onClick={onRunCitationReview} disabled={!canRunCitationReview || reviewBusy}
                aria-label={t('aiReview')}
                className="flex h-7 items-center gap-1 rounded-md bg-(--brand) px-2 text-[10px] font-bold text-(--on-brand) transition-colors hover:bg-(--brand-hover) disabled:opacity-50">
