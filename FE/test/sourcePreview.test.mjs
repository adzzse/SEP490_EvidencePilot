...
import { getSourceDownloadUrl } from '../src/utils/student/sourceDownload.js';

test('source preview uses the authenticated download route and releases its blob URL', async () => {
  const sourceLibraryContent = await readFile(new URL('../src/components/Student/SourceLibraryContent.jsx', import.meta.url), 'utf8');
  const fileViewer = await readFile(new URL('../src/components/features/FileViewerModal.jsx', import.meta.url), 'utf8');

  assert.match(sourceLibraryContent, /fileUrl: `\/api\/documents\/\$\{src\.id\}\/download`/);
...
test('instructor source library previews stored PDFs through the authenticated viewer', async () => {
  const sourceLibrary = await readFile(new URL('../src/components/Instructor/SourceLibraryPanel.jsx', import.meta.url), 'utf8');

  assert.match(sourceLibrary, /import FileViewerModal from '\.\.\/features\/FileViewerModal'/);
  assert.match(sourceLibrary, /import useUndoDelete/);
  // rationale: preview is unconditional (parity with Project/Collection detail) —
...
test('metadata-only source exposes the failed upstream download URL', async () => {
  ...
  assert.match(sourceLibraryContent, /metadataFetchedDescription/);
  assert.match(sourceLibraryContent, /sourceDownloadFailureReason/);
  assert.match(sourceLibraryContent, /href=\{sourceDownloadUrl\}/);
});
